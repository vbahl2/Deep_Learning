...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
       ...
       # print("In correct function")
        ...
        hidden_layer = np.dot(X,W1) + b1 #hidden_layer = X dot W = N X H
                                           # N X H dot (H x 1) = N x 1
        #hidden layer (with bias) = N x H
        # output = (N x H) x (H x C) = (N x C) + (C,)
        
        #Relu the hidden_layer:
        #hidden_layer = []
        
        relu = np.maximum(0,hidden_layer)
        #hidden_layer = np.abs(hidden_layer_weights)
        
        
        output_layer = np.dot(relu,W2)+ b2
   
        scores = output_layer 
        #softmax = np.exp(output_layer) / np.sum(np.exp(output_layer))
   
        #scores = -np.log(softmax)
        #scores = np.argmax(softmax,axis = 1)
        ########################################################################
        #                              END OF YOUR CODE                        #
        ########################################################################
       ...
       # loss += (.5 * reg * np.sum(W1 * W1)) + (.5 * reg + np.sum(W2 * W2))
        ...
        #Softmax gradient
        dscores = prob
        dscores[np.arange(N),y[:]] -= 1 # prob[np.arange(prob.shape[0]),y[:]] -= 1
        dscores /= N 
        ...
        relu_grad = dscores.dot(W2.T)
        relu_grad[relu == 0] = 0
        
        #relu_grad[dscores]
        #prob /= N
        
        grads['W1'] = X.T.dot(relu_grad)
        grads['b1'] = np.sum(relu_grad, axis=0, keepdims=True)
    ...
    def predict(self, X):
        ...
        ########################################################################
        hidden_layer = hidden_layer = X.dot(self.params['W1']) + self.params['b1']
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
        ...
        best_val = -1
        all_classifiers = []
        learning_rates = [5e-4, 1e3, 5e3]
        regularization_strengths = [.1, .5, .75]
        ...
        grid_search = [(x,y,z) for x in batch_size for y in learning_rates for z in regularization_strengths] 
        np.random.seed(42)
        ...
        #for b,l,r in grid_search:
        
        candidate_net = TwoLayerNet(input_size, h, num_classes)
        candidate_net.train(X_train, y_train, X_val, y_val, num_iters=1000, batch_size=200,learning_rate=l , learning_rate_decay=0.95, reg=r, verbose=True)

        val_acc = (candidate_net.predict(X_val) == y_val).mean()
        print('Validation accuracy: ', val_acc)"""

        """results[(b,l,r)]=val_acc
